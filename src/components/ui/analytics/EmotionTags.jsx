      ...
      {topMoods.length === 0 ? (
        ...
        <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-3">
          {topMoods.map((item, i) => {
            ...
            return (
            <motion.div key={item.moodId || i} variants={fadeUp} className="flex items-center gap-3">
              <span className="text-base flex-shrink-0">{mood?.emoji || '📝'}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-ink-700 dark:text-ink-300">{mood?.label || 'Unknown'}</span>
                ...
                <div className="w-full h-1.5 bg-earth-200 dark:bg-ink-700 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    style={{ backgroundColor: mood?.color || '#B69FCC' }}
                  />
                </div>
              </div>
            </motion.div>
